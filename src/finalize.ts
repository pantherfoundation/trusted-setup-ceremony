...
const BEACON_HASH = "0x81d94f995b977ba0ecff48f8a6687aeb90025f4142743d7135bcf9751195541d";
...
function applyRandomBeacon(lastContributionFolder: string): void {
  ...
  const lastFolderPath = path.join(contributionRootFolder, lastContributionFolder);
  ...
  if (zkeyFiles.length === 0) {
    throw new Error(`No zkey files found in the last contribution folder: ${lastContributionFolder}`);
  }
  ...
  for (const zkeyFile of zkeyFiles) {
    ...
    const beaconHashNoPrefix = BEACON_HASH.startsWith('0x') ? BEACON_HASH.substring(2) : BEACON_HASH;
    ...
    let r1csFile = r1csFiles.find(file => path.basename(file, '.r1cs') === circuitName);

    // If no exact match, try to find one with similar name
    if (!r1csFile) {
      console.log(`⚠️ No exact matching r1cs file found for ${zkeyFile}. Looking for partial matches...`);
      r1csFile = r1csFiles.find(file =>
        file.toLowerCase().includes(circuitName.toLowerCase()) ||
        circuitName.toLowerCase().includes(path.basename(file, '.r1cs').toLowerCase())
      );
    }

    // If still no match, let user choose manually
    if (!r1csFile) {
      console.error(`❌ Error: Could not find matching r1cs file for ${zkeyFile}`);
    ...
    const vkeyPath = path.join(finalFolderPath, `${path.basename(zkeyFile, '.zkey')}_verification_key.json`);
...
function createBeaconMetadataFile(timestamp: string): void {
  const finalFolderPath = getFinalFolderPath();
  const metadataPath = path.join(finalFolderPath, "BeaconRandomnessMetadata.json");
...
function createAttestationFile(timestamp: string): void {
  ...
  const zkeyFiles = fs.readdirSync(finalFolderPath).filter(file => file.endsWith(".zkey"));

  if (zkeyFiles.length === 0) {
    throw new Error(`No zkey files found in the final folder: ${FINAL_FOLDER_NAME}`);
  }
  ...
  try {
    ...
    if (!latestFolder) {
      ...
      if (existingFolders.length === 0) {
        throw new Error("No contributions found. Cannot finalize the ceremony.");
      }
